...
const handler = (req, res) => {
  if (req.method === 'POST') {
    ...
    const { email, name, mobile, nid, password } = req.body
  ...
  if (req.method === 'GET') {
    ...
    return User.find({ $or: [{ createdBy: new ObjectId(userId) }, { _id: new ObjectId(userId) }] })
      .then(users =>
        res.send({
          status: 'ok',
          users: users.map(user => ({ ...user._doc, self: user._id == userId })),
          count: users.length,
        })
      )
      .catch(error =>
